...
export default class Tab extends Component {
  ...
  render() {
    const { icon, label, active, containerStyle, testID, ...other } = this.props

    return (
      <TouchableWithoutFeedback onPress={this._handleTabPress} testID={testID} {...other}>
        <View
          style={[
            styles.container,
            containerStyle,
  ...
  _renderIcon = () => {
    const mode = this._getModeString()
    const { active, icon, activeIcon, iconStyle } = this.props
    ...
    return (
      ...
      >
        <View ref="_bnic" collapsable={false} style={iconStyle}>
